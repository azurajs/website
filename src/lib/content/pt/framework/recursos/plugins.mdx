---
title: Plugins
description: Guia completo de plugins do AzuraJS - estenda sua aplicação com recursos poderosos integrados
category: Recursos
icon: lucide:plug
order: 10
---

# Plugins 🔌

O AzuraJS vem com um ecossistema completo de plugins contendo **17 plugins integrados** que adicionam funcionalidades poderosas às suas aplicações sem precisar de dependências externas. Todos os plugins são acessíveis através do export `azurajs/plugins` e seguem uma API consistente e fácil de usar.

## Visão Geral

Plugins no AzuraJS são funções middleware que podem ser aplicadas globalmente ou em rotas específicas. Eles são projetados para ser:

- **Zero Dependências**: Construídos inteiramente com módulos nativos do Node.js
- **Type-Safe**: Suporte completo ao TypeScript com definições de tipo detalhadas
- **Componíveis**: Combine múltiplos plugins para funcionalidades complexas
- **Performáticos**: Otimizados para uso em produção
- **Extensíveis**: Fácil de criar plugins personalizados

## Plugins Disponíveis

### Plugins de Segurança

#### 🛡️ Helmet

Adiciona headers de segurança para proteger sua aplicação de vulnerabilidades web comuns.

```typescript
import { AzuraClient } from "azurajs";
import { helmet } from "azurajs/plugins";

const app = new AzuraClient();

app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      defaultSrc: ["'self'"],
      styleSrc: ["'self'", "'unsafe-inline'"],
      scriptSrc: ["'self'"],
      imgSrc: ["'self'", "data:", "https:"]
    }
  },
  hsts: {
    maxAge: 31536000,
    includeSubDomains: true,
    preload: true
  },
  frameguard: { action: 'deny' },
  noSniff: true,
  xssFilter: true,
  hidePoweredBy: true
}));
```

**Opções:**
- `contentSecurityPolicy`: Configurar diretivas CSP
- `hsts`: Configurações do HTTP Strict Transport Security
- `frameguard`: Configuração do X-Frame-Options
- `noSniff`: Ativar X-Content-Type-Options: nosniff
- `xssFilter`: Ativar X-XSS-Protection
- `hidePoweredBy`: Remover header X-Powered-By
- `dnsPrefetchControl`: Controlar DNS prefetching
- `referrerPolicy`: Definir Referrer-Policy

#### 🌐 CORS

Habilite Cross-Origin Resource Sharing para sua API.

```typescript
import { cors } from "azurajs/plugins";

app.use(cors({
  origins: ["http://localhost:3000", "https://example.com"],
  methods: ["GET", "POST", "PUT", "DELETE", "PATCH"],
  allowedHeaders: ["Content-Type", "Authorization"],
  exposedHeaders: ["X-Total-Count"],
  credentials: true,
  maxAge: 3600
}));
```

**Opções:**
- `origins`: Array de origens permitidas (use `["*"]` para todas)
- `methods`: Métodos HTTP permitidos
- `allowedHeaders`: Headers de requisição permitidos
- `exposedHeaders`: Headers expostos ao navegador
- `credentials`: Permitir cookies e headers de autorização
- `maxAge`: Duração do cache de preflight (segundos)

#### 🔒 CSRF

Proteja contra ataques de Cross-Site Request Forgery.

```typescript
import { csrf } from "azurajs/plugins";

app.use(csrf({
  secret: process.env.CSRF_SECRET,
  cookie: {
    httpOnly: true,
    secure: true,
    sameSite: 'strict'
  },
  ignoreMethods: ['GET', 'HEAD', 'OPTIONS']
}));

// Use nas suas rotas
app.get('/form', (req, res) => {
  res.send(`
    <form method="POST" action="/submit">
      <input type="hidden" name="_csrf" value="${req.csrfToken()}">
      <button>Enviar</button>
    </form>
  `);
});
```

**Recursos:**
- Geração automática de tokens
- Armazenamento baseado em cookies ou sessão
- Métodos ignorados configuráveis
- Validação de token

#### 🔐 Autenticação JWT

Sistema completo de autenticação JWT com assinatura, verificação e middleware.

```typescript
import { jwtMiddleware, createJWT, verifyJWT } from "azurajs/plugins";

// Criar um token JWT
const token = createJWT(
  { userId: 123, role: 'admin' },
  { 
    secret: process.env.JWT_SECRET,
    expiresIn: 3600,      // 1 hora
    algorithm: 'HS256',
    issuer: 'myapp',
    audience: 'myapp-users'
  }
);

// Proteger rotas
app.use('/api/protegido', jwtMiddleware({
  secret: process.env.JWT_SECRET,
  credentialsRequired: true,
  algorithm: 'HS256',
  getToken: (req) => {
    const auth = req.headers.authorization;
    if (auth && auth.startsWith('Bearer ')) {
      return auth.substring(7);
    }
    return null;
  }
}));

// Acessar token decodificado
app.get('/api/protegido/perfil', (req, res) => {
  const user = req.user; // Payload do JWT
  res.json({ user });
});

// Verificar um token manualmente
try {
  const payload = verifyJWT(token, { 
    secret: process.env.JWT_SECRET 
  });
  console.log(payload);
} catch (error) {
  console.error('Token inválido:', error.message);
}
```

**Recursos:**
- Suporta algoritmos HS256, HS384 e HS512
- Claims padrão JWT (exp, iat, nbf, iss, aud)
- Extração customizada de token
- Acesso type-safe ao payload
- Zero dependências externas

### Plugins de Performance

#### 🗜️ Compressão

Comprima automaticamente respostas com gzip, deflate ou Brotli.

```typescript
import { compression } from "azurajs/plugins";

app.use(compression({
  threshold: 1024,      // Comprimir apenas respostas > 1KB
  level: 6,             // Nível de compressão (0-9, padrão: 6)
  filter: (contentType) => {
    // Comprimir conteúdo baseado em texto
    return /json|text|javascript|css|xml|svg/.test(contentType);
  }
}));
```

**Opções:**
- `threshold`: Tamanho mínimo da resposta para comprimir (bytes)
- `level`: Nível de compressão (0-9, maior = melhor compressão mas mais lento)
- `filter`: Função para determinar quais content types comprimir

**Benefícios:**
- Reduz largura de banda em até 70%
- Negociação automática de encoding (aceita: gzip, deflate, br)
- Filtragem inteligente por content-type

#### 🏷️ ETag

Habilite cache HTTP com geração e validação de ETag.

```typescript
import { etag } from "azurajs/plugins";

app.use(etag({
  algorithm: 'sha1',  // Algoritmo de hash
  weak: true          // Usar ETags fracos (W/"...")
}));
```

**Recursos:**
- Geração automática de ETag
- Retorna 304 Not Modified quando apropriado
- Suporta ETags fortes e fracos
- Reduz transferência de dados para recursos não modificados

#### 🗂️ Arquivos Estáticos

Sirva arquivos estáticos com recursos de cache e segurança.

```typescript
import { serveStatic } from "azurajs/plugins";

app.use('/public', serveStatic('./public', {
  maxAge: 86400000,        // Cache de 1 dia
  index: 'index.html',     // Arquivo padrão
  dotfiles: 'ignore',      // Ignorar dotfiles
  etag: true,
  lastModified: true,
  extensions: ['html', 'htm']
}));
```

**Opções:**
- `maxAge`: Duração do cache em milissegundos
- `index`: Arquivo índice padrão
- `dotfiles`: Como lidar com dotfiles ('allow', 'ignore', 'deny')
- `etag`: Habilitar geração de ETag
- `lastModified`: Enviar header Last-Modified
- `extensions`: Extensões de arquivo a tentar

### Plugins de Manipulação de Dados

#### 📦 Sessão

Gerencie sessões de usuário com memória ou store customizado.

```typescript
import { session } from "azurajs/plugins";

app.use(session({
  secret: process.env.SESSION_SECRET,
  name: 'sessionId',
  cookie: {
    maxAge: 86400000,    // 24 horas
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production',
    sameSite: 'strict'
  },
  rolling: true,         // Estender sessão na atividade
  resave: false,
  saveUninitialized: false
}));

// Usar sessões
app.post('/login', (req, res) => {
  req.session.userId = user.id;
  req.session.role = user.role;
  res.json({ success: true });
});

app.get('/perfil', (req, res) => {
  if (!req.session.userId) {
    return res.status(401).json({ error: 'Não autorizado' });
  }
  res.json({ 
    userId: req.session.userId,
    role: req.session.role 
  });
});

// Destruir sessão
app.post('/logout', (req, res) => {
  req.session.destroy();
  res.json({ success: true });
});
```

**Exemplo de Store Customizado:**

```typescript
// Implementação de store Redis
class RedisStore {
  constructor(private redis: RedisClient) {}

  async get(id: string) {
    const data = await this.redis.get(`session:${id}`);
    return data ? JSON.parse(data) : null;
  }

  async set(id: string, data: any) {
    await this.redis.setex(`session:${id}`, 86400, JSON.stringify(data));
  }

  async destroy(id: string) {
    await this.redis.del(`session:${id}`);
  }
}

app.use(session({
  secret: process.env.SESSION_SECRET,
  store: new RedisStore(redisClient)
}));
```

#### 📤 Multipart

Lide com upload de arquivos com multipart/form-data.

```typescript
import { multipart } from "azurajs/plugins";

app.post('/upload', multipart({
  limits: { 
    fileSize: 10 * 1024 * 1024,  // 10MB
    files: 5                       // Máximo 5 arquivos
  },
  allowedTypes: ['image/jpeg', 'image/png', 'application/pdf'],
  uploadDir: './uploads'
}), (req, res) => {
  const files = req.files;
  const fields = req.fields;
  
  res.json({ 
    uploaded: files.length,
    files: files.map(f => ({
      name: f.originalFilename,
      size: f.size,
      type: f.mimetype
    })),
    fields 
  });
});
```

**Opções:**
- `limits`: Limites de tamanho e contagem
- `allowedTypes`: Array de tipos MIME permitidos
- `uploadDir`: Diretório para salvar arquivos enviados
- `keepExtensions`: Manter extensões de arquivo originais

#### 📏 Limite de Body

Limite o tamanho do body da requisição para prevenir abuso.

```typescript
import { bodyLimit } from "azurajs/plugins";

app.use(bodyLimit({
  limit: '10mb',  // String ou número (bytes)
  onLimit: (req, res) => {
    res.status(413).json({ 
      error: 'Payload muito grande',
      maxSize: '10mb'
    });
  }
}));
```

### Plugins de Infraestrutura

#### ⚡ Rate Limiting

Previna abuso de API com limitação de taxa de requisições.

```typescript
import { rateLimit } from "azurajs/plugins";

app.use('/api', rateLimit({
  windowMs: 60000,         // 1 minuto
  max: 100,                // Máximo 100 requisições por janela
  message: 'Muitas requisições, tente novamente mais tarde',
  statusCode: 429,
  skipSuccessfulRequests: false,
  skipFailedRequests: false,
  keyGenerator: (req) => req.ip  // Usar IP como chave
}));
```

**Recursos:**
- Janelas de tempo configuráveis
- Rastreamento por IP ou chave customizada
- Respostas de erro customizadas
- Opção de pular requisições bem-sucedidas/falhadas

#### 🔄 Proxy

Proxy reverso para microserviços e gateways de API.

```typescript
import { proxyPlugin } from "azurajs/plugins";

app.use('/api/users', proxyPlugin('http://users-service:4001', {
  pathRewrite: {
    '^/api/users': '/users'
  },
  headers: {
    'X-Custom-Header': 'AzuraJS-Proxy'
  },
  timeout: 5000,
  logLevel: 'info',
  onProxyReq: (proxyReq, req) => {
    proxyReq.setHeader('X-Real-IP', req.ip);
  },
  onProxyRes: (proxyRes, req, res) => {
    console.log(`Proxy: ${req.url} -> ${proxyRes.statusCode}`);
  }
}));
```

#### ⚡ Circuit Breaker

Implemente padrão de tolerância a falhas para aplicações resilientes.

```typescript
import { circuitBreaker, CircuitBreaker } from "azurajs/plugins";

// Como middleware
app.use('/api/externo', circuitBreaker({
  failureThreshold: 5,    // Abrir após 5 falhas
  timeout: 60000,         // Resetar após 1 minuto
  onOpen: () => {
    console.log('Circuito aberto - parando requisições');
  },
  onClose: () => {
    console.log('Circuito fechado - retomando requisições');
  }
}));

// Como classe utilitária
const dbBreaker = new CircuitBreaker({
  failureThreshold: 3,
  timeout: 30000
});

app.get('/usuarios', async (req, res) => {
  try {
    const usuarios = await dbBreaker.execute(async () => {
      return await db.query('SELECT * FROM users');
    });
    res.json(usuarios);
  } catch (error) {
    if (error.message === 'Circuit breaker is open') {
      return res.status(503).json({ 
        error: 'Serviço temporariamente indisponível' 
      });
    }
    throw error;
  }
});
```

**Estados:**
- **Fechado**: Operação normal, requisições passam
- **Aberto**: Muitas falhas, requisições são bloqueadas
- **Semi-Aberto**: Testando se o serviço recuperou

### Tempo Real e Monitoramento

#### 📊 Server-Sent Events (SSE)

Habilite push em tempo real do servidor com SSE.

```typescript
import { SSEManager, createSSEHandler } from "azurajs/plugins";

const sse = new SSEManager();

// Endpoint SSE
app.get('/eventos', createSSEHandler(sse));

// Broadcast para todos os clientes
app.post('/notificar', (req, res) => {
  sse.broadcast({
    event: 'notificacao',
    data: { message: 'Olá a todos!' }
  });
  res.json({ enviado: true });
});

// Enviar para cliente específico
app.post('/notificar/:clientId', (req, res) => {
  sse.send(req.params.clientId, {
    event: 'mensagem-privada',
    data: { message: 'Só para você' }
  });
  res.json({ enviado: true });
});

// Métricas em tempo real
setInterval(() => {
  sse.broadcast({
    event: 'metricas',
    data: {
      cpu: process.cpuUsage(),
      memoria: process.memoryUsage(),
      uptime: process.uptime()
    }
  });
}, 1000);
```

**Uso no cliente:**

```javascript
const eventSource = new EventSource('/eventos');

eventSource.addEventListener('notificacao', (event) => {
  const data = JSON.parse(event.data);
  console.log('Notificação:', data.message);
});

eventSource.addEventListener('metricas', (event) => {
  const data = JSON.parse(event.data);
  atualizarDashboard(data);
});
```

#### 🩺 Health Check

Monitore a saúde da aplicação e dependências.

```typescript
import { healthCheck } from "azurajs/plugins";

app.use('/health', healthCheck({
  checks: {
    database: async () => {
      try {
        await db.ping();
        return { 
          status: 'healthy',
          latency: 5,
          connections: db.pool.totalCount
        };
      } catch (error) {
        return { 
          status: 'unhealthy',
          error: error.message 
        };
      }
    },
    redis: async () => {
      try {
        await redis.ping();
        return { status: 'healthy' };
      } catch (error) {
        return { status: 'unhealthy', error: error.message };
      }
    },
    apiExterna: async () => {
      const inicio = Date.now();
      try {
        await fetch('https://api.example.com/health');
        return { 
          status: 'healthy',
          tempoResposta: Date.now() - inicio
        };
      } catch (error) {
        return { status: 'unhealthy' };
      }
    }
  },
  timeout: 5000,  // 5 segundos de timeout para checks
  cache: 10000    // Cachear resultados por 10 segundos
}));
```

**Formato da resposta:**

```json
{
  "status": "healthy",
  "timestamp": "2026-02-04T10:30:00.000Z",
  "uptime": 3600.5,
  "checks": {
    "database": {
      "status": "healthy",
      "latency": 5,
      "connections": 10
    },
    "redis": {
      "status": "healthy"
    },
    "apiExterna": {
      "status": "healthy",
      "tempoResposta": 150
    }
  }
}
```

#### 🔢 Request ID

Adicione identificadores únicos às requisições para rastreamento.

```typescript
import { requestId } from "azurajs/plugins";

app.use(requestId({
  header: 'X-Request-ID',
  generator: () => crypto.randomUUID()
}));

app.use((req, res, next) => {
  console.log(`[${req.id}] ${req.method} ${req.url}`);
  res.set('X-Request-ID', req.id);
  next();
});
```

#### ⏱️ Timeout

Previna requisições travadas com timeouts automáticos.

```typescript
import { timeout } from "azurajs/plugins";

app.use(timeout({
  duration: 30000,  // 30 segundos
  onTimeout: (req, res) => {
    console.error(`Timeout de requisição: ${req.method} ${req.url}`);
    res.status(408).json({ 
      error: 'Timeout de requisição',
      message: 'O servidor não recebeu uma requisição completa a tempo'
    });
  }
}));
```

## Composição de Plugins

Combine múltiplos plugins para funcionalidade poderosa:

### Configuração Pronta para Produção

```typescript
import { AzuraClient } from "azurajs";
import { 
  helmet, 
  cors, 
  compression, 
  rateLimit, 
  requestId,
  timeout,
  bodyLimit,
  healthCheck
} from "azurajs/plugins";

const app = new AzuraClient();

// Camada de segurança
app.use(helmet());
app.use(cors({ 
  origins: [process.env.FRONTEND_URL],
  credentials: true 
}));
app.use(bodyLimit({ limit: '10mb' }));

// Camada de performance
app.use(compression({ threshold: 1024 }));
app.use(timeout({ duration: 30000 }));

// Camada de monitoramento
app.use(requestId());
app.use('/health', healthCheck({
  checks: {
    db: async () => ({ status: await db.isHealthy() ? 'healthy' : 'unhealthy' })
  }
}));

// Rate limiting
app.use('/api', rateLimit({
  windowMs: 60000,
  max: 100
}));

// Suas rotas...
app.get('/api/usuarios', (req, res) => {
  res.json({ usuarios: [] });
});

app.listen(3000);
```

### API de Autenticação

```typescript
import { 
  helmet, 
  cors, 
  csrf,
  jwtMiddleware,
  createJWT,
  rateLimit,
  bodyLimit
} from "azurajs/plugins";

const app = new AzuraClient();

app.use(helmet());
app.use(cors({ origins: [process.env.FRONTEND_URL], credentials: true }));
app.use(bodyLimit({ limit: '1mb' }));

// Rate limit em endpoints de autenticação
app.use('/auth', rateLimit({
  windowMs: 900000,  // 15 minutos
  max: 5             // 5 tentativas
}));

// Endpoint de login
app.post('/auth/login', async (req, res) => {
  const { email, password } = req.body;
  
  const usuario = await validarCredenciais(email, password);
  if (!usuario) {
    return res.status(401).json({ error: 'Credenciais inválidas' });
  }
  
  const token = createJWT(
    { userId: usuario.id, role: usuario.role },
    { 
      secret: process.env.JWT_SECRET,
      expiresIn: 3600  // 1 hora
    }
  );
  
  res.json({ token, usuario: { id: usuario.id, email: usuario.email } });
});

// Rotas protegidas
app.use('/api', jwtMiddleware({
  secret: process.env.JWT_SECRET,
  credentialsRequired: true
}));

app.get('/api/perfil', (req, res) => {
  res.json({ usuario: req.user });
});
```

### Serviço de Upload de Arquivos

```typescript
import { 
  helmet,
  csrf,
  multipart,
  bodyLimit,
  jwtMiddleware
} from "azurajs/plugins";

const app = new AzuraClient();

app.use(helmet());
app.use(bodyLimit({ limit: '50mb' }));
app.use(csrf());

app.post('/upload',
  jwtMiddleware({ secret: process.env.JWT_SECRET }),
  multipart({
    limits: { 
      fileSize: 50 * 1024 * 1024,
      files: 10
    },
    allowedTypes: ['image/*', 'application/pdf', 'video/mp4']
  }),
  async (req, res) => {
    const files = req.files;
    
    // Processar e salvar arquivos
    const arquivosSalvos = await Promise.all(
      files.map(arquivo => salvarNoStorage(arquivo))
    );
    
    res.json({ 
      enviados: arquivosSalvos.length,
      arquivos: arquivosSalvos 
    });
  }
);
```

### Gateway de Microserviços

```typescript
import { 
  helmet,
  cors,
  rateLimit,
  proxyPlugin,
  circuitBreaker,
  requestId,
  healthCheck
} from "azurajs/plugins";

const gateway = new AzuraClient();

gateway.use(helmet());
gateway.use(cors({ origins: ['*'] }));
gateway.use(requestId());
gateway.use(rateLimit({ max: 1000, windowMs: 60000 }));

// Health check para o gateway
gateway.use('/health', healthCheck({
  checks: {
    servicoUsuarios: async () => {
      try {
        await fetch('http://users-service:4001/health');
        return { status: 'healthy' };
      } catch {
        return { status: 'unhealthy' };
      }
    },
    servicoPedidos: async () => {
      try {
        await fetch('http://orders-service:4002/health');
        return { status: 'healthy' };
      } catch {
        return { status: 'unhealthy' };
      }
    }
  }
}));

// Proxy para serviços com circuit breaker
gateway.use('/usuarios', 
  circuitBreaker({ failureThreshold: 5, timeout: 60000 }),
  proxyPlugin('http://users-service:4001', {
    pathRewrite: { '^/usuarios': '' }
  })
);

gateway.use('/pedidos',
  circuitBreaker({ failureThreshold: 5, timeout: 60000 }),
  proxyPlugin('http://orders-service:4002', {
    pathRewrite: { '^/pedidos': '' }
  })
);

gateway.listen(3000);
```

## Criando Plugins Customizados

Você pode criar seus próprios plugins seguindo o mesmo padrão:

```typescript
import type { RequestHandler } from "azurajs";

interface OpcoesPluginCustomizado {
  habilitado?: boolean;
  prefixo?: string;
}

export function pluginCustomizado(opcoes: OpcoesPluginCustomizado = {}): RequestHandler {
  const { habilitado = true, prefixo = 'custom' } = opcoes;
  
  return (req, res, next) => {
    if (!habilitado) {
      return next();
    }
    
    // Lógica do seu plugin
    req.customData = `${prefixo}-${Date.now()}`;
    
    res.on('finish', () => {
      // Limpeza ou logging
      console.log(`Requisição finalizada: ${req.customData}`);
    });
    
    next();
  };
}
```

**Plugin avançado com operações assíncronas:**

```typescript
export function pluginAssincrono(opcoes: Opcoes): RequestHandler {
  return async (req, res, next) => {
    try {
      // Operações assíncronas
      const dados = await buscarAlgunsDados();
      req.pluginData = dados;
      next();
    } catch (error) {
      next(error);
    }
  };
}
```

## Melhores Práticas

### 1. A Ordem Importa

Aplique plugins na ordem correta para funcionalidade ideal:

```typescript
// ✅ Ordem correta
app.use(helmet());           // 1. Segurança primeiro
app.use(cors());             // 2. CORS
app.use(bodyLimit());        // 3. Limites de parsing do body
app.use(compression());      // 4. Compressão
app.use(requestId());        // 5. Rastreamento de requisição
app.use('/api', rateLimit()); // 6. Rate limiting
// ... suas rotas
```

### 2. Configuração Específica do Ambiente

```typescript
const isProducao = process.env.NODE_ENV === 'production';

app.use(helmet({
  hsts: isProducao ? { maxAge: 31536000 } : false
}));

app.use(session({
  cookie: {
    secure: isProducao,
    sameSite: isProducao ? 'strict' : 'lax'
  }
}));
```

### 3. Ajuste de Performance

```typescript
// Comprimir apenas respostas grandes
app.use(compression({ 
  threshold: 2048  // Mínimo de 2KB
}));

// Cachear health checks
app.use('/health', healthCheck({
  checks: { /* ... */ },
  cache: 30000  // Cache por 30 segundos
}));

// Otimizar rate limiting para alto tráfego
app.use(rateLimit({
  windowMs: 1000,  // Janelas de 1 segundo
  max: 10,         // Janelas menores, mais verificações
}));
```

### 4. Tratamento de Erros

```typescript
import { circuitBreaker } from "azurajs/plugins";

app.use('/api/externo', circuitBreaker({
  failureThreshold: 5,
  timeout: 60000,
  onOpen: () => {
    // Alertar sistema de monitoramento
    servicoMonitoramento.alerta('Circuit breaker aberto');
  }
}));

// Handler de erro global
app.use((error, req, res, next) => {
  console.error(`[${req.id}] Erro:`, error);
  res.status(500).json({ 
    error: 'Erro interno do servidor',
    requestId: req.id 
  });
});
```

### 5. Checklist de Segurança

Para deploys de produção, certifique-se de ter:

- ✅ Helmet para headers de segurança
- ✅ CORS configurado corretamente
- ✅ Rate limiting em todos os endpoints públicos
- ✅ Limites de tamanho do body
- ✅ Proteção CSRF para operações que mudam estado
- ✅ Autenticação JWT ou sessão
- ✅ Timeouts de requisição
- ✅ Endpoint de health check

## Impacto na Performance

### Resultados de Benchmark

- **Compressão**: 60-70% de redução de largura de banda
- **ETag**: 90% de redução para recursos cacheados
- **Request ID**: &lt; 0.5ms overhead
- **JWT Middleware**: &lt; 2ms overhead
- **Rate Limit**: &lt; 1ms overhead (in-memory)

### Uso de Memória

- **Session Store**: ~50 bytes por sessão
- **SSE Manager**: ~1KB por cliente conectado
- **Rate Limit**: ~100 bytes por IP rastreado
- **Circuit Breaker**: ~100 bytes por instância

## Próximos Passos 📖

- [Melhores Práticas de Segurança](/docs/pt/framework/avancado/seguranca)
- [Deploy em Produção](/docs/pt/framework/avancado/producao)
- [Otimização de Performance](/docs/pt/framework/avancado/performance)
- [Desenvolvimento de Plugins Customizados](/docs/pt/framework/avancado/plugins-customizados)
